...
void BreakoutGame::UpdateDimensions(unsigned int width, unsigned int height) {
	this->width = width;
	this->height = height;
...
void BreakoutGame::ProcessInput(float deltaTime) {
...
void BreakoutGame::Update(float deltaTime) {
	Ball->Move(deltaTime, this->width, Player->position, PLAYER_SIZE);
	this->DoCollisions();
	BallTrail->Update(deltaTime, *Ball, 2, glm::vec2(Ball->radius));
...
Direction BreakoutGame::VectorDirection(glm::vec2 target) {
	glm::vec2 compass[] = {
...
Collision BreakoutGame::CheckCollision(BallClass &ball, GameObject &block) {
